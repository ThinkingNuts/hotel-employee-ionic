...
export class BaiduMapPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams) {
    ...
    let lng:number = navParams.get("lng");
    let lat:number = navParams.get("lat");
    this.point = new BMap.Point(lng, lat);//坐标可以通过百度地图坐标拾取器获取 
  ...
  ionViewDidEnter() {
    ...
    this.marker = new BMap.Marker(this.point, { icon: this.myIcon });
    this.map.addOverlay(this.marker);
    map.centerAndZoom(this.point, 18);//设置中心和地图显示级别

    console.log("ionViewDidEnter::finish");
  ...
  getLocationByLatLon() {
    console.log("getLocationByLatLon");
